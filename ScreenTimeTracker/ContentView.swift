...
struct ContentView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            let displayed = showAllHistory
                ? Array(tracker.sessions.reversed())
                : Array(tracker.sessions.suffix(10).reversed())
            ...
            if tracker.sessions.isEmpty {
                ...
                Button(showAllHistory
                       ? "▲ Show less"
                       : "▼ Show all \(tracker.sessions.count) sessions") {
