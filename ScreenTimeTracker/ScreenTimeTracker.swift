...
// ── IOKit battery reader (no shell needed) ────────────────────────────────────
func readBatteryIOKit() -> (Int, Bool) {
    ...
    if service != 0 {
        ...
        if let chg = IORegistryEntryCreateCFProperty(
...
class ScreenTimeTracker: ObservableObject {
    ...
    init() {
        ...
        // Read battery synchronously via IOKit before first render
    ...
    // ── Battery (IOKit — no shell required) ───────────────────────────────────
